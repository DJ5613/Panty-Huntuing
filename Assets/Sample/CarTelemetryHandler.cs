...
public class CarTelemetryHandler : MonoBehaviour
{
    ...
    private void Awake()
    {
        // Сбрасываем положение капсулы в 0 при старте
    ...
    private void ResetCapsulePosition()
    {
        if (vehicleTransform != null)
        {
            // Сбрасываем позицию и вращение
            vehicleTransform.position = Vector3.zero;
            vehicleTransform.rotation = Quaternion.identity;

            // Если есть Rigidbody, сбрасываем также физические параметры
            if (rigidbody != null)
            {
